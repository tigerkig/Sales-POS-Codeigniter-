...
EOD;
    ...
    {
				$spout_excel_writer_format = (!defined("SPOUT_EXCEL_WRITER_CELL_FORMAT")) ? 0 : SPOUT_EXCEL_WRITER_CELL_FORMAT;
        $registeredStyles = $this->getRegisteredStyles();

        $content = '<cellXfs count="' . count($registeredStyles) . '">';

        foreach ($registeredStyles as $style) {
            $content .= '<xf numFmtId="'.$spout_excel_writer_format.'" fontId="' . $style->getId() . '" fillId="0" borderId="0" xfId="0"';
